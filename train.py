...
def validate_model(model,validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_msg,global_state,writer,num_example=2):
    model.eval()
    count = 0
    console_width = 80
    with torch.no_grad():
...
def get_model(config,vocab_src_len,vocab_tgt_len):
    model= build_transformer(vocab_src_len,vocab_tgt_len,config["seq_len"],config["seq_len"],config["d_model"],h=1)
    return model
def train_model(config):
    ...
    for epoch in range(initial_epoch,config["num_epochs"]):
        ...
        },model_directory +"/"+ model_filename+str(epoch)+".pt")
